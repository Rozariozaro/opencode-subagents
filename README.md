...
└───┬──────────┬──────────┬──────────┬─────────────────────┘
    │          │          │          │
┌───▼────┐ ┌──▼─────┐ ┌──▼─────┐ ┌──▼──────────┐
│ SCOUT  │ │BUILDER │ │AUDITOR │ │ DOCUMENTER  │
│        │ │        │ │        │ │             │
│Read-   │ │Write · │ │Approve/│ │Single-pass  │
│only    │ │Edit ·  │ │Reject /│ │doc writer · │
│analyst │ │Build · │ │Clarify │ │md/docs only │
│        │ │Test ·  │ │        │ │no src edits │
│        │ │Docs*   │ │        │ │             │
└────────┘ └────────┘ └────────┘ └─────────────┘
    │
...
\*\*Documenter is delegated to by Conductor for documentation-only tasks (README, CHANGELOG, docs/); auditor is skipped for those tasks.
...
| `researcher` | subagent | `github-copilot/gpt-5-mini`        | 0.1  | Technical research analyst; framework comparisons, OSS discovery, API research              |
...
Phase 2: Implementation     → @builder with full plan + context + validation commands
                               (doc-only tasks routed to @documenter, skipping @auditor)
...
| Documenter skips auditor                 | Documentation-only tasks don't need a code quality gate; conductor self-verifies the output              |
...
| Documenter uses cheap model              | Token efficiency for documentation tasks; no need for highest-capability model                           |
...
### A few future expansion agents

tester.md -> automated test generation/execution
repair.md -> focused bug fixing
optimizer.md -> performance/refactor specialist
security.md -> vuln scanning and secure coding
observer.md -> logs/runtime monitoring
librarian.md -> memory/context compression
router.md -> model/tool routing decisions
...
_Built with [OpenCode](https://opencode.ai) · Powered by Anthropic Claude, OpenAI GPT, and Google Gemini_
